...
row = 0
col = 0
for header in ["file","Record", "NameI","NameII"]:
...
for xmiFileName in xmiFileNames:
    ...
    for paragraph in cas.select('gr.ilsp.types.Paragraph'):
        ...
        if text:
            ...
            names = set()
            for name in cas.select_covered("gr.ilsp.nlp.dis.titles.ruta.Names.NameI", paragraph):
                nameText = name.get_covered_text().strip()
                if nameText not in names:
                    col = 0
                    worksheet.write(row, col, xmiFileName)
                    col = col + 1
                    worksheet.write(row, col, text)
                    col = col + 1
                    worksheet.write(row, col, name.get_covered_text())
                    names.add(nameText)
            if len(names) > 0:
                row = row + 1
